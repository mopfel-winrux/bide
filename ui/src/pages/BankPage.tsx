...
const ALL_CATEGORIES: (ItemCategory | 'all')[] = [
  'all', 'raw-material', 'processed', 'equipment', 'food', 'gem', 'rune', 'potion', 'seed', 'tablet', 'bones', 'misc',
];
...
export function BankPage() {
  ...
  const sortedItems = useMemo(() => {
    let entries = Object.entries(bank).filter(([, q]) => q > 0);
  ...
  const selectedQty = selectedItem ? bank[selectedItem] : undefined;

  // Clear selection if item no longer in bank
  useEffect(() => {
    if (selectedItem && !(selectedItem in bank)) {
            ...
            <div className="grid grid-cols-[repeat(auto-fill,minmax(140px,1fr))] gap-3">
              {sortedItems.map(([iid, qty]) => (
                <BankItem
                  ...
                  itemId={iid}
                  qty={qty}
                  selected={selectedItem === iid}
                  onClick={() => setSelectedItem(selectedItem === iid ? null : iid)}
                />
              ))}
            </div>
          )}
        ...
        <div className="w-[320px] shrink-0 hidden lg:block">
          <div className="sticky top-6">
            {selectedItem && selectedQty ? (
              <ItemDetailPanel key={selectedItem} itemId={selectedItem} qty={selectedQty} />
            ) : (
              <div className="bg-[#111827] border border-[#1e293b] rounded-lg p-8 text-center text-gray-600 text-sm">
