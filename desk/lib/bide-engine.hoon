::  lib/bide-engine.hoon — game tick + combat event processing
::
/-  *bide
/+  bide-xp, bide-skills, bide-items, bide-combat, bide-monsters
/+  bide-equipment, bide-modifiers, bide-potions, bide-prayers
/+  bide-slayer, bide-specials, bide-dungeons, bide-farming
/+  bide-pets, bide-spells, bide-bank
|%
+$  card  card:agent:gall
::
++  process-tick
  |=  [gs=game-state =bowl:gall]
  ...
  =/  act  u.aa
  ?-  -.act
    %skilling  (process-skill-tick gs act bowl)
    %combat    `gs  :: combat uses its own Behn timers
    %dungeon   `gs  :: dungeons use combat timers
  ==
::
++  process-skill-tick
  |=  [gs=game-state act=active-action =bowl:gall]
  ^-  (quip card game-state)
  ?>  ?=(%skilling -.act)
  ...
  =.  items.bank.gs  (deposit:bide-bank items.bank.gs produced)
...
++  process-combat-events
  ...
  ?~  aa  `gs
  =/  act  u.aa
  ?:  ?=(%dungeon -.act)  (process-dungeon-events gs act bowl)
  ?.  ?=(%combat -.act)  `gs
  ...
  ?:  p-first
    ...
    =.  items.bank.gs  (deposit:bide-bank items.bank.gs loot-items)
...
++  process-dungeon-events
  ...
  ?:  p-first
    ...
    ?:  (gte next-room total-rooms)
      ...
      =.  items.bank.gs  (deposit:bide-bank items.bank.gs reward-items)
      =.  rng-seed.gs  seed
      =.  hitpoints-current.player.gs  p-hp
      =.  active-action.gs  ~
      `gs
